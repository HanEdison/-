  ...
  if(isset($_POST["mySent"]))
        {
           ...
           "uploadfile/lunwen/" .iconv('utf-8','gb2312',$name));
	$XM=$_POST['XM'];
	$TGLX=$_POST['TGLX'];
    $SZDW=$_POST['SZDW'];
	$SZKS=$_POST['SZKS'];
    ...
    $file=$_FILES["file"]["name"];  
	   if ($XM == '') {
			                ...
			                echo "<script>alert('投稿类型不能为空！');history.back(-1);</script>";
						} else if ($SZDW == '') {
							echo "<script>alert('所在单位不能为空！');history.back(-1);</script>";
						} else if ($TXDZ == '') {
							echo "<script>alert('通讯地址不能为空！');history.back(-1);</script>";
						} else if ($LXDH == '') {
							echo "<script>alert('联系电话不能为空！');history.back(-1);</script>";
						} else if ($file == '') {
							echo "<script>alert('上传文章不能为空！');history.back(-1);</script>";
						} else { 
